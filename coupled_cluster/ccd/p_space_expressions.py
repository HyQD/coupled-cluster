...
def compute_eta(occ,virt,h,u,Dpq,Dpqrs):
    ...
    eta[occ,virt] = eta_jb.reshape(Dpq[occ,occ].shape[0],Dpq[virt,virt].shape[0])
...
def compute_A_ajib(rho_ij,rho_ab):
    ...
    A_ajib = A_ajib.reshape(rho_ab.shape[0]*rho_ij.shape[0],rho_ab.shape[0]*rho_ij.shape[0])
    return A_ajib 
...
def compute_R_ai(occ,virt,h,u,Dpq,Dpqrs):
    ...
    R_ai += 0.5*np.einsum('pris,pras->ai',Dpqrs[:,:,occ,:],u[:,:,virt,:],optimize=True)
    R_ai -= 0.5*np.einsum('arqs,irqs->ai',Dpqrs[virt,:,:,:],u[occ,:,:,:],optimize=True)
...
def compute_R_ia(occ,virt,h,u,Dpq,Dpqrs):
    ...
    R_ia += 0.5*np.einsum('pras,pris->ia',Dpqrs[:,:,virt,:],u[:,:,occ,:],optimize=True)
    R_ia -= 0.5*np.einsum('irqs,arqs->ia',Dpqrs[occ,:,:,:],u[virt,:,:,:],optimize=True)
