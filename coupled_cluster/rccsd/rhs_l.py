...
def compute_l_1_amplitudes(f, u, t1, t2, l1, l2, o, v, np, out=None):
    ...
    I22_l1 += np.einsum(
        "al,lijk->ijka", t1, I21_l1
    )

    del I21_l1

    rhs += np.einsum(
        "iljk,kjla->ia", I3_l1, u[o,o,o,v]
    )

    del I3_l1

    I25_l1 += np.einsum(
        "likb,abjl->ijka", I2_l1, t2
    )

    I25_l1 += np.einsum(
        "ilkb,bajl->ijka", I2_l1, t2
    )

    rhs += np.einsum(
        "ijkb,jkab->ia", I25_l1, u[o,o,v,v]
    )
    ...
    I31_l1 += np.einsum(
        "kjib,bakj->ia", I2_l1, t2
    )

    I4_l1 = np.zeros((no, no, no, nv))

    I4_l1 += np.einsum(
        "bi,jkab->ijka", t1, u[o,o,v,v]
    )
    ...
    I14_l1 += np.einsum(
        "klib,ablj->ijka", I6_l1, t2
    )

    I24_l1 = np.zeros((no, no, no, nv))

    I24_l1 += np.einsum(
        "ijlb,ablk->ijka", I6_l1, t2
    )
    ...
    I14_l1 -= np.einsum(
        "bk,ijab->ijka", t1, I11_l1
    )

    rhs -= np.einsum(
        "jkba,kijb->ia", I11_l1, I2_l1
    )
    ...
    I13_l1 += np.einsum(
        "ablk,ijba->ijkl", t2, u[o,o,v,v]
    )

    I14_l1 += np.einsum(
        "al,lijk->ijka", t1, I13_l1
    )
    ...
    I14_l1 -= np.einsum(
        "bcjk,iacb->ijka", t2, u[o,v,v,v]
    )

    rhs += np.einsum(
        "ijkb,jkba->ia", I14_l1, l2
    )
    ...
    I15_l1 += np.einsum(
        "ikac,bcjk->ijab", l2, t2
    )

    I15_l1 += np.einsum(
        "ikca,cbjk->ijab", l2, t2
    )

    rhs -= np.einsum(
        "ijbc,jbac->ia", I15_l1, u[o,v,v,v]
    )
    ...
    I17_l1 += np.einsum(
        "kjbc,kica->ijab", I16_l1, l2
    )

    rhs += np.einsum(
        "ijbc,jbca->ia", I17_l1, u[o,v,v,v]
    )

    del I17_l1

    I31_l1 -= np.einsum(
        "jb,jiab->ia", l1, I16_l1
    )
    ...
    I18_l1 += np.einsum(
        "di,bacd->iabc", t1, u[v,v,v,v]
    )

    rhs += np.einsum(
        "jbca,jibc->ia", I18_l1, l2
    )
    ...
    I22_l1 -= np.einsum(
        "ljba,likb->ijka", I20_l1, I2_l1
    )

    rhs += np.einsum(
        "ijkb,jkba->ia", I22_l1, u[o,o,v,v]
    )

    del I22_l1

    I35_l1 += np.einsum(
        "kjab,kiab->ij", I20_l1, u[o,o,v,v]
    )
    ...
    I23_l1 += np.einsum(
        "ai,jkla->ijkl", t1, u[o,o,o,v]
    )

    I24_l1 += np.einsum(
        "al,ijlk->ijka", t1, I23_l1
    )

    del I23_l1

    I24_l1 -= np.einsum(
        "bi,jakb->ijka", t1, u[o,v,o,v]
    )

    rhs += np.einsum(
        "jikb,kjab->ia", I24_l1, l2
    )
    ...
    I26_l1 += np.einsum(
        "cj,iacb->ijab", t1, u[o,v,v,v]
    )

    rhs -= np.einsum(
        "jkba,ikjb->ia", I26_l1, I2_l1
    )
    ...
    I34_l1 += np.einsum(
        "ci,iabc->ab", t1, I28_l1
    )

    rhs += np.einsum(
        "bc,ibca->ia", I27_l1, I28_l1
    )
    ...
    I35_l1 += np.einsum(
        "ak,ikja->ij", t1, I33_l1
    )

    rhs -= np.einsum(
        "jk,kija->ia", I30_l1, I33_l1
    )
...
def compute_l_2_amplitudes(f, u, t1, t2, l1, l2, o, v, np, out=None):
    ...
    I36_l2 += np.einsum(
        "jikc,kcab->ijab", I1_l2, u[o,v,v,v]
    )
    ...
    I29_l2 += np.einsum(
        "kiac,kjcb->ijab", I28_l2, u[o,o,v,v]
    )
    ...
    I34_l2 += np.einsum(
        "bj,jiab->ia", t1, I33_l2
    )

    del I33_l2

    I35_l2 -= np.einsum(
        "jb,ia->ijab", I34_l2, l1
    )
    ...
    I53_l2 += np.einsum(
        "ablk,jiab->ijkl", t2, u[o,o,v,v]
    )

    rhs += np.einsum(
        "jikl,klba->ijab", I53_l2, l2
    )

    del I53_l2

    I54_l2 += np.einsum(
        "jiab,ablk->ijkl", l2, t2
    )

    rhs += np.einsum(
        "jikl,klba->ijab", I54_l2, u[o,o,v,v]
    )

    del I54_l2

    rhs += np.einsum(
        "jicd,dcab->ijab", l2, u[v,v,v,v]
    )
