...
def compute_l_1_amplitudes(
    ...
    if intermediates == None:
        ...
        Loovv = intermediates.Loovv
        Lvovv = intermediates.Lvovv
        Looov = intermediates.Looov

        Hoo = intermediates.Hoo
        Hov = intermediates.Hov
        Hvv = intermediates.Hvv
        Hovvo = intermediates.Hovvo
        Hovov = intermediates.Hovov
        Hvvvo = intermediates.Hvvvo
        Hovoo = intermediates.Hovoo
        Hvovv = intermediates.Hvovv
        Hooov = intermediates.Hooov

    # l1 equations
    r_l1 = 2.0 * Hov
    r_l1 += ndot("ie,ea->ia", l1, Hvv)
    r_l1 -= ndot("im,ma->ia", Hoo, l1)
    r_l1 += ndot("ieam,me->ia", Hovvo, l1, prefactor=2.0)
    r_l1 += ndot("iema,me->ia", Hovov, l1, prefactor=-1.0)
    r_l1 += ndot("imef,efam->ia", l2, Hvvvo)
    r_l1 -= ndot("iemn,mnae->ia", Hovoo, l2)
    r_l1 -= ndot("eifa,ef->ia", Hvovv, build_Gvv(t2, l2), prefactor=2.0)
    r_l1 -= ndot("eiaf,ef->ia", Hvovv, build_Gvv(t2, l2), prefactor=-1.0)
    r_l1 -= ndot("mina,mn->ia", Hooov, build_Goo(t2, l2), prefactor=2.0)
    r_l1 -= ndot("imna,mn->ia", Hooov, build_Goo(t2, l2), prefactor=-1.0)
...
def compute_l_2_amplitudes(
    ...
    if intermediates == None:
        ...
        Hoooo = intermediates.Hoooo
        Hvvvv = intermediates.Hvvvv
    ...
    r_l2 += Loovv
    r_l2 += ndot("ia,jb->ijab", l1, Hov, prefactor=2.0)
    r_l2 -= ndot("ja,ib->ijab", l1, Hov)
    r_l2 += ndot("ijeb,ea->ijab", l2, Hvv)
    r_l2 -= ndot("im,mjab->ijab", Hoo, l2)
    r_l2 += ndot("ijmn,mnab->ijab", Hoooo, l2, prefactor=0.5)
    r_l2 += ndot("ijef,efab->ijab", l2, Hvvvv, prefactor=0.5)
    r_l2 += ndot("ie,ejab->ijab", l1, Hvovv, prefactor=2.0)
    r_l2 += ndot("ie,ejba->ijab", l1, Hvovv, prefactor=-1.0)
    r_l2 -= ndot("mb,jima->ijab", l1, Hooov, prefactor=2.0)
    r_l2 -= ndot("mb,ijma->ijab", l1, Hooov, prefactor=-1.0)
    r_l2 += ndot("ieam,mjeb->ijab", Hovvo, l2, prefactor=2.0)
    r_l2 += ndot("iema,mjeb->ijab", Hovov, l2, prefactor=-1.0)
    r_l2 -= ndot("mibe,jema->ijab", l2, Hovov)
    r_l2 -= ndot("mieb,jeam->ijab", l2, Hovvo)
    r_l2 += ndot("ijeb,ae->ijab", Loovv, build_Gvv(t2, l2))
    r_l2 -= ndot("mi,mjab->ijab", build_Goo(t2, l2), Loovv)
