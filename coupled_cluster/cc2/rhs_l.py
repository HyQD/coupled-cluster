...
def compute_l_1_amplitudes(f, f_t, u_t, t_1, t_2, l_1, l_2, o, v, np, out=None):
   ...
   # print(l_1)
    ...
    l_1_print = np.sort(abs(l_1.flatten()))[::-1]
       
    print("Lambda Singles Sorted")    
    print(l_1_print)
    
    add_s1_l(f_t, o, v, out, np=np)       #1           
    add_s2a_l(f_t, l_1, o, v, out, np=np) #2 f(v,v)         
    add_s2b_l(f_t, l_1, o, v, out, np=np) #3 f(o,o)         
    add_s3a_l(u_t, l_1, o, v, out, np=np) #4           
    add_s4a_l(u_t, l_2, o, v, out, np=np) #5(vvvo)       
    add_s4b_l(u_t, l_2, o, v, out, np=np) #6(ooov)   
    #add_s6d_l(u_t, l_2, t_2, o, v, out, np=np)       
    add_s7_l(u_t, l_1, t_2, o, v, out, np=np)   #7(oovv)   
    #add_s9a_l(u_t, l_2, t_2, o, v, out, np=np)      
    #add_s9c_l(u_t, l_2, t_2, o, v, out, np=np)         
    #add_s10a_l(f, l_2, t_2, o, v, out, np=np)       
    #add_s10b_l(f, l_2, t_2, o, v, out, np=np)       
    add_s10c_l(u_t, l_1, t_2, o, v, out, np=np) #8 -0.5(oovv)   
    add_s10d_l(u_t, l_1, t_2, o, v, out, np=np) #9 -0.5(oovv)   #########
    #add_s10e_l(u_t, l_2, t_2, o, v, out, np=np)     
    #add_s10f_l(u_t, l_2, t_2, o, v, out, np=np)      
    #add_s10g_l(u_t, l_2, t_2, o, v, out, np=np)  
...
def compute_l_2_amplitudes(f, f_t, u_t, t_1, t_2, l_1, l_2, o, v, np, out=None):
    ...
    l_2_print = np.sort(abs(l_2.flatten()))[::-1]       
    print(l_2_print[0:20])

    add_d1_l(u_t, o, v, out, np=np) #1  *
    #add_d2a_l(u_t, l_2, o, v, out, np=np) 
    #add_d2b_l(u_t, l_2, o, v, out, np=np)  
    add_d2c_l(f, l_2, o, v, out, np=np) #2 *
    add_d2d_l(f, l_2, o, v, out, np=np) #3 *
    #add_d2e_l(u_t, l_2, o, v, out, np=np) 
    #add_d3a_l(u_t, t_2, l_2, o, v, out, np=np) 
    #add_d3b_l(u_t, t_2, l_2, o, v, out, np=np) 
    #add_d3c_l(u_t, t_2, l_2, o, v, out, np=np) 
    #add_d3d_l(u_t, t_2, l_2, o, v, out, np=np) 
    #add_d3e_l(u_t, t_2, l_2, o, v, out, np=np) 
    #add_d3f_l(u_t, t_2, l_2, o, v, out, np=np) 
    #add_d3g_l(u_t, t_2, l_2, o, v, out, np=np) 
    add_d5a_l(u_t, l_1, o, v, out, np=np) #4  *  
    add_d5b_l(u_t, l_1, o, v, out, np=np) #5  *
    add_d7a_l(f_t, l_1, o, v, out, np=np) #6  *
