...
class CC2(CoupledCluster):
    ...
    def __init__(self, system, include_singles=True, **kwargs):
        ...
        #super().__init__(system, mixer=AlphaMixer, verbose=False)
    ...
    def compute_energy(self):
        ...
        print(self.compute_reference_energy())
        print("correction")
        print(energy) 
    ...
    def compute_t_amplitudes(self):
        ...
        f_transform, u_transform = self.T1_transform_integrals()
...
#        asym = lambda X: X - np.swapaxes(X, 0,1) - np.swapaxes(X, 2,3) + np.swapaxes(np.swapaxes(X,0,1), 2,3)
#        self.t_2 = 0.25*asym(self.t_2)
    ...
    def T1_transform_integrals(self):
        ...
        h_transform = np.einsum("pr,qs,rs -> pq", x_transform, y_transform, self.h_0)  
         
        u_transform = np.einsum('pt,qu,rm,sn,tumn->pqrs', x_transform, x_transform, y_transform, y_transform, self.u_0, optimize="greedy") 
       
        asym = lambda X: X - np.swapaxes(X, 0,1) - np.swapaxes(X, 2,3) + np.swapaxes(np.swapaxes(X,0,1), 2,3)
 
 #       u_transform = 0.25*asym(u_transform)
 
        f_transform = self.f.copy()
        f_transform = self.construct_fock_matrix_new(h_transform, u_transform)  
       
        return f_transform, u_transform 
