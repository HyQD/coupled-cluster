...
def lagrangian_functional(f, f_transform, u_transform, t_1, t_2, l_1, l_2, o, v, np):
    ...
    energy = np.tensordot(f_transform[v, o], l_1, axes=((0, 1), (1, 0)))    #l_1*f     

    term = np.tensordot(l_1, t_2, axes=((0, 1), (3, 1)))  # ai
    energy += np.tensordot(f_transform[o, v], term, axes=((0, 1), (1, 0)))   #l1*t2*f   
    ...
    energy += np.tensordot(
        term, u_transform[v, o, v, v], axes=((0, 1, 2, 3), (0, 2, 3, 1)))  #u*l1*t2       
    ...
    energy += np.tensordot(l_2, temp_dckl, axes=((0, 1, 2, 3), (3, 2, 0, 1))) #f*t2*l2 


    temp_dclk = 0.5 * np.tensordot(f[v, v], t_2, axes=((1), (0)))
    
    energy += np.tensordot(temp_dclk, l_2, axes=((0, 1, 2, 3), (2, 3, 0, 1))) #f*t2*l2 
