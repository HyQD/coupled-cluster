...
Dabij = np.zeros((nvirt,nvirt,nocc,nocc)) #lib.direct_sum('ai,bj->abij',Dai,Dai)
    ...
    for j in range(0,nocc):
        for a in range(nocc,nocc+nvirt):
            for b in range(nocc,nocc+nvirt):
                ...
                Dabij[a-nocc,b-nocc,i,j] = F[i,i]+F[j,j]-F[a,a]-F[b,b]
...
while( max(norm_residual_t1,norm_residual_t2) > conv_tol and (iters < max_iters)):
...
while( max(norm_residual_l1,norm_residual_l2) > conv_tol and (iters < max_iters)):
