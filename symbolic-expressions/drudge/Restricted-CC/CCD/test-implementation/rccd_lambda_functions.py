...
def l2_rhs(f,u,l,t):
    ...
    rhs += 2 * np.einsum(
        "ca,ijcb->ijab", f[v,v], l
    )

    rhs += 2 * np.einsum(
        "cb,ijac->ijab", f[v,v], l
    )
    ...
    rhs -= 2 * np.einsum(
        "ikac,cjbk->ijab", l, u[v,o,v,o]
    )

    rhs -= 2 * np.einsum(
        "ikbc,cjka->ijab", l, u[v,o,o,v]
    )

    rhs -= 2 * np.einsum(
        "ikcb,cjak->ijab", l, u[v,o,v,o]
    )

    rhs -= 2 * np.einsum(
        "kjac,ickb->ijab", l, u[o,v,o,v]
    )

    rhs -= 2 * np.einsum(
        "kjca,icbk->ijab", l, u[o,v,v,o]
    )

    rhs -= 2 * np.einsum(
        "kjcb,icka->ijab", l, u[o,v,o,v]
    )

    rhs += 4 * np.einsum(
        "ikac,cjkb->ijab", l, u[v,o,o,v]
    )

    rhs += 4 * np.einsum(
        "kjcb,icak->ijab", l, u[o,v,v,o]
    )

    rhs += 2 * np.einsum(
        "ijcd,cdab->ijab", l, u[v,v,v,v]
    )

    rhs -= 2 * np.einsum(
        "ik,kjab->ijab", f[o,o], l
    )

    rhs -= 2 * np.einsum(
        "jk,ikab->ijab", f[o,o], l
    )

    rhs += 2 * np.einsum(
        "kmab,ijkm->ijab", l, u[o,o,o,o]
    )
